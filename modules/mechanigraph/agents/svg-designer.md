...
在编写任何 SVG 代码前，必须回查 `paper-master-4ss/modules/mechanigraph/` 的矢量规范库，并在设计记录开头输出 `## 参考库回查`。

- 本 agent 所属模块: `paper-master-4ss/modules/mechanigraph/`。
- 必读模块参考:
  - `modules/mechanigraph/references/design_system.md`（坐标系统、组件尺寸、防穿透曲线数学公式、间隙标准）
  - `modules/mechanigraph/references/drawing_patterns_reference.md`（8 大构型标准几何样板代码）
  - `modules/mechanigraph/resources/academic_defs.svg`（标准箭头 marker 定义）
- 必用工具: `modules/mechanigraph/scripts/render_svg.py`（无头渲染与视觉核查）
...
[生成完整纯矢量 SVG]
      │
      ▼
[运行 python3 modules/mechanigraph/scripts/render_svg.py input.svg output.png]
      │
      ▼
[检查输出结果与视觉布局]
  1. 四周留白与居中是否对称？
