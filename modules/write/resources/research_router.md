...
```bash
python3 modules/write/scripts/example_extractor.py \
...
进入新正文撰写前，主流程必须先把以下输入包传给 `modules/write/agents/structure-writing-consultant.md`，生成 `writing-dispatch-plan`：
...
随后主流程按 `writing-dispatch-plan` 把单个任务包传给 `modules/write/agents/chapter-draft-writer.md`。每个 draft writer 输出必须保存为 `paper-workspace/_logs/agents/write-[YYYY-MM-DD]/paper-write-chapter-draft-writer--{task_id}.md`，不得覆盖其它任务。

所有 writer 输出完成后，主流程必须在 write 模块目录运行拼接脚本。若当前目录是 skill 包内的 `modules/write/`，所有 `paper-workspace` 参数必须使用项目工作区绝对路径，避免误写到 skill 包内部：
...
- `modules/write/resources/research_router.md`：总入口，负责方法协议和范式选择
- `modules/write/chapters/`：具体章节写作标准
- `modules/write/examples/`：范文和提示词模板
- `modules/write/agents/structure-writing-consultant.md`：写作规划与 `writing-dispatch-plan` agent
- `modules/write/agents/chapter-draft-writer.md`：按单个任务包撰写章节、分节或段落草稿 agent
- `modules/write/agents/style-consultant.md`：全文文风总控 agent
- `modules/write/scripts/method_router.py`：自动生成方法路由报告
- `modules/write/scripts/assemble_drafts.py`：按 `writing-dispatch-plan` 拼接多个 writer 输出并生成 source map
- `modules/write/scripts/writing_scanner.py`：语言反模式和合规扫描
- `modules/write/scripts/complexity_analyzer.py`：文本复杂度诊断
