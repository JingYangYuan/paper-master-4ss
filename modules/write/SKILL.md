...
```bash
cd "$(dirname "/path/to/paper-master-4ss/modules/write/SKILL.md")"
```
...
| 触发场景 | 可派发 agent |
|---|---|
| 写作规划、章节/小节拆分、分段派发表 | `modules/write/agents/structure-writing-consultant.md` |
| 章节、分节或段落正文草稿撰写 | `modules/write/agents/chapter-draft-writer.md` |
| 问题意识、论点、证据和理论对话 | `modules/write/agents/argument-consultant.md` |
| 文献、数据、访谈、案例和正文嵌入 | `modules/write/agents/material-integration-consultant.md` |
| 全文文风总控、术语统一、扫描器改写循环 | `modules/write/agents/style-consultant.md` |
| ch01-ch10 章节标准复核 | `modules/write/agents/chapter-standard-reviewer.md` |
...
14. 完整质量门交给 `modules/check/`：scanner 循环与 `chapter-standard-reviewer` 完成后，不得把全文审稿、编辑首筛、拒稿风险诊断或投稿前综合自检做成 write 内部检查。语言扫描与复杂度诊断仍留 write。将正文净稿、扫描报告、source map、证据边界和目标期刊信息按 `master/handoff-checklists.md` 的 `write -> check` 交接。
...
第三层，写作风格判断。详见 `modules/write/resources/writing_style_subrouter.md`。
...
详细写作标准、硬性门槛、迭代规则和反模式已下沉到 `modules/write/chapters/` 目录。处理具体章节的写作或润色请求时，应先 Read 对应 chapter 文件，再 Read 对应 example 文件。

| 论文要素 | 标准代号 | 详细标准 | 结构模板与提示词 |
|---------|---------|-----------|-----------------|
| 研究路由 | — | `modules/write/resources/research_router.md` | — |
| 选题与形式逻辑 | — | `modules/write/chapters/ch01-form-logic.md` + `modules/write/chapters/ch02-topic-material.md` | — |
| 标题设计 | [G1]-[G4] | `modules/write/chapters/ch02-topic-material.md` | — |
| 摘要与关键词 | [A1]-[A5] | `modules/write/chapters/ch02-topic-material.md` | — |
| 引言 | [I1]-[I7] | `modules/write/chapters/ch03-genesis-question.md` | `modules/write/examples/intro_example.md` |
| 文献综述 | [L1]-[L6] | `master/literature-review-protocol.md` + `modules/write/chapters/ch04-lit-review.md` + `modules/write/chapters/ch05-lit-technique.md` | `modules/write/examples/lit_review_example.md` |
| 研究方法与数据 | [M0]-[M5] | `modules/write/chapters/ch06-material-method.md` | — |
| 经验分析 / 研究发现 | [E1]-[E5] | `modules/write/chapters/ch07-organizing-materials.md` | `modules/write/examples/findings_example.md` |
| 理论对话 / 分析框架 | [T1]-[T5] | `modules/write/chapters/ch08-theory-dialogue.md` | `modules/write/examples/framework_example.md` |
| 结论与讨论 | [C1]-[C6] | `modules/write/chapters/ch09-conclusion.md` | `modules/write/examples/conclusion_example.md` |
| 参考文献格式 | [R1]-[R3] | `modules/write/chapters/ch10-submission-ethics.md` | — |
...
3. 快速决策：材料组织逻辑、开头风格、理论对话路径等决策表集中在 `modules/write/resources/cheatsheet.md`。
...
| 论文部分 | 文件 | 内容 |
|---------|------|------|
| 引言 | `modules/write/examples/intro_example.md` | 社会学量化、质性、管理世界案例三种引言模式 |
| 文献综述 | `modules/write/examples/lit_review_example.md` | 证据驱动综述：可选结构、段落示意与交付前检查 |
| 分析框架 | `modules/write/examples/framework_example.md` | 维度分解型、理论对话型、机制链型、矩阵型框架 |
| 研究发现 | `modules/write/examples/findings_example.md` | 量化实证、质性田野、案例研究三种发现模式 |
| 结论与讨论 | `modules/write/examples/conclusion_example.md` | 社会学范式和管理世界范式结论模式 |
...
| 文件 | 功能 |
|------|------|
| `modules/write/resources/research_router.md` | 总入口，负责方法协议和发表范式双层判断 |
| `modules/write/resources/writing_style_subrouter.md` | 第三层风格路由：学理化梯度与小处入手策略 |
| `modules/write/resources/glossary.md` | 关键术语及定义 |
| `modules/write/resources/patterns.md` | 写作技法与模式 |
| `modules/write/resources/cheatsheet.md` | 快速参考、决策表和检查清单 |
| `modules/write/chapters/` | 分章节写作标准 |
| `modules/write/examples/` | 范文和提示词 |
| `modules/write/scripts/method_router.py` | 方法路由器 |
| `modules/write/scripts/example_extractor.py` | 范文截取器——按路由结果截取对应 example 章节 |
| `modules/write/scripts/assemble_drafts.py` | 多个 `chapter-draft-writer` 输出的拼接、source map 和完整性检查 |
| `modules/write/scripts/writing_scanner.py` | 写作扫描器 |
| `modules/write/scripts/complexity_analyzer.py` | 文本复杂度分析器 |
