...
用法：
  python3 modules/write/scripts/method_router.py paper.md
  python3 modules/write/scripts/method_router.py paper.md --json
  python3 modules/write/scripts/method_router.py paper.md --out route_report.md
"""
...
def to_markdown(report: RouteReport) -> str:
    ...
    lines.append("\n## 六、使用建议\n")
    lines.append("先用本报告确定方法路径，再读取 `modules/write/resources/research_router.md` 确定目标期刊风格，最后进入 `modules/write/chapters/` 和 `modules/write/examples/` 调用章节标准与提示词。")
    return "\n".join(lines) + "\n"
