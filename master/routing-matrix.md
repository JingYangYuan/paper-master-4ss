...
| 用户意图 | 内部模块 | 入口 |
|---|---|---|
| 选题、理论锚点、概念/规范/阐释理论设计、研究问题、实证或混合研究设计、PAP | design | `modules/design/SKILL.md`（先确认 FRAME/STORM/DESIGN/FULL 与研究取向，不预设理论或实证） |
| 文献检索、文献综述、文献地图、研究空白、假设推导、CNKI/Scholar | lit | `modules/lit/SKILL.md`（综述正文按 `master/literature-review-protocol.md` 交接 write，唯一落点 `05-writing/literature-review.md`） |
| 已有材料转论文结构、大纲、证据映射、缺口报告 | outline | `modules/outline/SKILL.md` |
| 数据清洗、描述统计、回归诊断(VIF/BP/DW/Hausman/弱IV/过度识别)、基准回归、Logit/Probit/Poisson/Tobit/Heckman、面板FE/RE/动态GMM、IV/2SLS/GMM、DID/事件研究/多期DID(交错处理)、RDD(局部平滑性/安慰剂/带宽)、PSM/CEM/IPW、SCM/SDID、分位数、中介/调节、时间序列(ARMA/VAR/VECM)、空间计量(SAR/SDM/SEM)、Bootstrap/稳健性、质性编码、混合方法。三语言模板(Stata/R/Python)自含完整可执行代码。 | analysis | `modules/analysis/SKILL.md` |
| 正文写作、章节改写、润色、语言扫描、复杂度诊断 | write | `modules/write/SKILL.md` |
| 全文审稿、编辑首筛、内容与形式联合检查、拒稿风险诊断、投稿前综合自检 | check | `modules/check/SKILL.md` |
| 投稿前的 Word、引文体例、模板、投稿包、cover letter、response letter | submission | `modules/submission/SKILL.md` |
| 更新知识边界、补充理论框架、补充写作范式、补充方法/流程协议、补充工具模板、从书籍/论文/笔记/方法手册/工具日志吸收知识，或为 update 模块自身生成待审核更新包 | update | `modules/update/SKILL.md` |
| 学术机制图、理论机制图、分析框架图、演进模型、政策网络、治理体系、纯矢量SVG、机制图复刻、机制图生成、因果图绘制（兼容 mechanismgraph / mechanigraph 拼写） | mechanigraph | `modules/mechanigraph/SKILL.md`（出图前 ask_user 确认风格偏好，出图后无头 Chrome 渲染自检） |
