...
- **机制图矢量自检（Mechanigraph）**：Antigravity 运行机制图生成时，调用 `modules/mechanigraph/scripts/render_svg.py` 驱动系统 Chrome 或无头模式渲染真实 PNG。若在纯云端/容器无图形环境中缺失 Chrome，脚本会安全降级并输出纯矢量 SVG 源码与元数据 JSON，提示在宿主中查看，不阻断主流程。
